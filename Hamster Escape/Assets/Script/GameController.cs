    ...
    public void SetUpGame()
    {
        ...
        for (int i = 0; i < data.question[index].Obj; i++)
        {
            ...
            else
            {
                if (i < data.question[index].Obj - 1)
                {
                   // int n = 0;
                    if (index <= 5)
                    {
                        ...
                        GameObject test = Instantiate(/*addItem[i]*/data.Obj[ran], new Vector2(i * 15f, 1f), Quaternion.identity);
                        test.transform.SetParent(hidd_tranform.transform);
                        targets.Add(test.transform.GetChild(0).transform);
                ...
                {
                    GameObject test = Instantiate(/*addItem[i]*/ data.Finish, new Vector2(i * 15f, 1f), Quaternion.identity);
                    test.transform.SetParent(hidd_tranform.transform);
                    targets.Add(test.transform.GetChild(0).transform);
